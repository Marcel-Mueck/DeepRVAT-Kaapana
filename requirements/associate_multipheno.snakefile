...
# conda = '. ~/.bashrc && conda deactivate && conda activate genopheno2'
conda_check = 'conda info | grep "active environment"'
cuda_visible_devices = 'echo CUDA_VISIBLE_DEVICES=$CUDA_VISIBLE_DEVICES'
py = 'python ~/git/genopheno/genopheno/aggregation_metrics/'

vcf = Path('/omics/odcf/analysis/OE0540_projects/ukbb/exomes/vcf')

experiment_name = Path.cwd().name

repeats_to_compare = config.get("repeats_to_compare", [1, 6])
...
rule combine_regression_chunks:
    input:
        testing = expand('{{phenotype}}/{{type}}/repeat_{{repeat}}/results/burden_associations_testing_{chunk}.parquet', chunk=range(n_regression_chunks)),
        replication = expand('{{phenotype}}/{{type}}/repeat_{{repeat}}/results/burden_associations_replication_{chunk}.parquet', chunk=range(n_regression_chunks))
    output:
        testing = '{phenotype}/{type}/repeat_{repeat}/results/burden_associations_testing.parquet',
        replication = '{phenotype}/{type}/repeat_{repeat}/results/burden_associations_replication.parquet'
    threads: 1
    resources:
        mem_mb = 2048,
        load = 2000
    shell:
        ' && '.join([
            conda_check,
            py + 'univariate_burden_regression.py combine-regression-results '
...
rule link_burdens:
    ...
    #         else f'--link-burdens ../../../{phenotypes[0]}/deeprvat/burdens/burdens.zarr '
    #     )
    threads: 8
    resources:
...
rule compute_burdens:
    ...
    output:
        ...
        #     ("finished" if wildcards.phenotype == phenotype[0] else "linked")
        # )
    # params:
    #     link_burdens = lambda wildcards: (
    #         '' if wildcards.phenotype == phenotypes[0]
    ...
    resources:
        ...
        load = 8000,
        gpus = 1
        # mem_mb = lambda wildcards: (32000 if wildcards.phenotype == phenotypes[0]
        #                             else 16384),
        # load = lambda wildcards: (32000 if wildcards.phenotype == phenotypes[0]
        #                           else 16000),
    shell:
        ' && '.join([
            conda_check,
...
#             for repeat in range(n_repeats) for bag in range(n_bags)
#         ],
#         config = '{phenotype}/deeprvat/repeat_0/config.yaml', #TODO make this more generic
#         # (the config from any repeat can be used cause they are all the same)
#     output:
